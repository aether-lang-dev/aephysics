...
| `aephysics.contact_solver_wide` | a colour's convex contacts four at a time: the constraint of four contacts as one single-precision record in lanes (`f32x4`, std.lanes -- the reference's SIMD path), the same prepare, warm start, solve, restitution and store, the bodies gathered into lanes and scattered back | `test_contact_solver_wide.ae` (one scene two ways -- the stack, the bounce with hit events, rolling resistance, a conveyor, a slide -- the lanes within a millimetre of the scalar solve, and the same lanes twice bit for bit) | [scalar 249 ms, lanes 162, the reference 161](bench/RESULTS.md#solver) |
